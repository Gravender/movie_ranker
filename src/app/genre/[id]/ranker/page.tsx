...
export default async function GenreRanker({ params }: Props) {
  ...
  const matches = await api.movie.getGenreMoviesToCompare.query({
  ...
  return (
    <div className="flex h-full w-full items-center justify-center gap-4 pt-4">
      {matches.length > 0 && (
        <RankMovie matches={matches} user_id={session.user.id} genre={genre} />
      )}
    </div>
  );
}
